...
<!-- ***
## JSON 서버 만들어보자
> https://minhanpark.github.io/%EC%8B%9C%EB%A6%AC%EC%A6%88/json-server-crud/ 참고했음
***

- json-server 설치  
  `npm i -D json-server`

- `package.json` 에 다음 명령어 추가
    ```json
     {
        "scripts": {
...
- `npm run server` 명령어 치면 `json-server` 만들어짐
- 기본 port는 `3000번`이지만,  
  다른 포트로 설정해주고 싶다면 스크립트에 `–port 옵션`을 붙여주면 됨  
  `json-server –watch db.json –port 4000` 이런 식으로
*** -->